...
def test_policy_precedence_normalization_and_checksum():
    ...
    policy, checksum = service.resolve_contact_policy(
        ...
        campaign_kind="promotional", purpose="ofertas_e_novidades",
    )
...
def test_revocation_does_not_expire_but_grant_does():
    ...
    assert service.resolve_applicable_consent({1: {"id": "r", "status": "revoked", "valid_until": expired}}, 1) == ("revoked", "r")
    assert service.resolve_applicable_consent({1: {"id": "g", "status": "granted", "valid_until": expired}}, 1) == ("unknown", "g")


def test_eligibility_keeps_policy_blocks_distinct():
    base = {
        "lead": {"id": 7, "persona_id": "p1", "telefone": "+55 51 99999-0000"},
        "selected_audience_id": "a1", "semantic_audience_id": "a1",
        "campaign_kind": "promotional", "consent_status": "granted",
        "provider_ready": True, "persona_id": "p1",
    }
...
def test_meta_mock_is_qa_only(monkeypatch):
    binding = {"provider": "meta_cloud", "connection_status": "connected", "provider_secret_ciphertext": None, "whatsapp_phone_number_id": None}
...
def test_preview_deduplicates_imports_and_freezes_a_stable_checksum(monkeypatch):
    ...
    batches = [{"id": "b1", "persona_id": "p1", "status": "completed"}, {"id": "b2", "persona_id": "p1", "status": "completed"}]
    leads = [{"id": 1, "persona_id": "p1", "telefone": "5551999990001", "nome": "Um"}, {"id": 2, "persona_id": "p1", "telefone": "5551999990002", "nome": "Dois"}]
    ...
    monkeypatch.setattr(service.supabase_client, "get_active_whatsapp_binding", lambda _id: {"provider": "meta_cloud", "connection_status": "connected", "provider_secret_ciphertext": "cipher", "whatsapp_phone_number_id": "123"})
    ...
    monkeypatch.setattr(service, "_latest_consents", lambda **_kwargs: {2: {"id": "c2", "status": "revoked"}})
    payload = {"persona_id": "p1", "audience_id": "a1", "import_batch_ids": ["b1", "b2"], "campaign_kind": "consent_request", "purpose": "ofertas_e_novidades", "provider": "meta_cloud", "template_name": "consentimento_v1"}
    first = service.preview_campaign(payload)
    second = service.preview_campaign(payload)
    assert first["counts"] == {"selected_unique": 2, "eligible": 1, "blocked": 1, "duplicate_rows_removed": 1}
